...
source("R/makebetas.R")
 ...
 for (i in 1:n.sims) {
   ...
   many.cov.betas <- gen_betas_many_cov2(n.epis, 5,5)


  N <- nrow(many.cov.betas)
  pcts[,i] <- runif(N, .05,1)
  #pcts <- seq(5,25,length.out = 25)/25

  gamma <- 1/3
  beta <- many.cov.betas$beta2/3
  ...
  dat <-subset(many.cov.betas, select=-beta2)
  #dat$K <- epi.mdl.normcurve.init(ecs)$K
...
save(pcts, fs, K.VC_linear, K.VC_rpart, K.VC_super, K.norm_linear, K.norm_rpart, K.norm_super,
     K.VC_alone, K.norm_alone, K.rpart_alone, K.linear_alone, K.super_alone, file="SIRMulitVar.RData")
